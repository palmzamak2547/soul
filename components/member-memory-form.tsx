    ...
    return (
      <div className="grid min-h-[calc(100dvh-78px)] place-items-center px-4 py-12">
        <motion.section animate={{ opacity: 1, scale: 1 }} className="w-full max-w-[600px] rounded-[30px] border border-[var(--line)] bg-white p-8 text-center shadow-[var(--shadow-soft)] sm:p-12" initial={{ opacity: 0, scale: .96 }}>
          ...
          <p className="mb-3 mt-7 [font-family:var(--font-mono)] text-[8px] font-bold uppercase tracking-[.15em] text-[var(--pink-strong)]">{state === "success" ? "MEMORY SAVED" : "MEMORY REMOVED"}</p>
  ...
  return (
    <div className="mx-auto w-full max-w-[1380px] px-4 py-6 sm:px-7 sm:py-9 lg:px-10">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--line)] pb-5">
        <Link className="inline-flex min-h-11 items-center gap-2 text-[9px] font-semibold text-[var(--muted)] transition hover:text-[var(--ink)]" href={`/member/cards/${cardId}`}><ArrowLeft size={16} /> กลับไปที่ {card.name}</Link>
        <div className="flex items-center gap-2"><button className="inline-flex min-h-10 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 text-[9px] font-semibold" onClick={() => setShowPreview((value) => !value)} type="button"><Eye size={15} /> {showPreview ? "ซ่อนตัวอย่าง" : "ดูตัวอย่าง"}</button><button className="hidden min-h-10 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 text-[9px] font-semibold sm:inline-flex" onClick={saveDraft} type="button"><FloppyDisk size={15} /> {draftSaved ? "บันทึกร่างแล้ว" : "เก็บเป็นร่าง"}</button></div>
      </header>
      ...
      <div className={`mt-7 grid gap-6 ${showPreview ? "xl:grid-cols-[minmax(0,1fr)_410px]" : "lg:grid-cols-[minmax(0,1fr)_330px]"}`}>
        <form className="min-w-0 rounded-[28px] border border-[var(--line)] bg-white p-5 shadow-[0_10px_35px_rgba(83,41,58,.045)] sm:p-8" onSubmit={save}>
          <div className="flex items-start gap-4"><span className="grid size-12 shrink-0 place-items-center rounded-2xl bg-[var(--blush)] text-[var(--pink)]"><NotePencil size={23} weight="fill" /></span><div><p className="m-0 [font-family:var(--font-mono)] text-[8px] font-bold uppercase tracking-[.15em] text-[var(--pink-strong)]">{mode === "create" ? "NEW MEMORY" : "EDIT MEMORY"}</p><h1 className="mb-0 mt-2 [font-family:var(--font-display)] text-[clamp(28px,4vw,42px)] font-semibold tracking-[-.055em]">{mode === "create" ? "เก็บวันนี้ไว้ให้อนาคต" : "แก้ไขเรื่องราวของคุณ"}</h1></div></div>
          ...
          <div className="mt-8 space-y-6">
            ...
            <Field label="ภาพประกอบ" optional supporting="JPG, PNG หรือ WEBP · สูงสุด 10 MB">
              {filePreview ? (
                <div className="relative aspect-[1.9] overflow-hidden rounded-[20px] border border-[var(--line)] bg-[var(--cream)]"><Image alt="ตัวอย่างภาพความทรงจำ" className="object-cover" fill sizes="(max-width: 1024px) 100vw, 700px" src={filePreview} unoptimized={filePreview.startsWith("blob:")} /><div className="absolute inset-x-3 bottom-3 flex items-center gap-2 rounded-[14px] border border-white/60 bg-white/85 p-2.5 backdrop-blur-md"><ImageSquare className="text-[var(--pink)]" size={17} /><span className="min-w-0 flex-1 truncate text-[8px] font-semibold">{fileName}</span><button aria-label="ลบภาพ" className="grid size-8 place-items-center rounded-full bg-[var(--ink)] text-white" onClick={() => { if (filePreview.startsWith("blob:")) URL.revokeObjectURL(filePreview); setFilePreview(""); setFileName(""); }} type="button"><X size={14} /></button></div></div>
              ) : (
                <label className="flex min-h-[160px] cursor-pointer flex-col items-center justify-center rounded-[20px] border border-dashed border-[#d8c2cb] bg-[var(--paper)] p-6 text-center transition hover:border-[var(--pink)] hover:bg-[#fff6f8]"><input accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={onFile} type="file" /><span className="grid size-11 place-items-center rounded-full bg-white text-[var(--pink)] shadow-sm"><UploadSimple size={20} /></span><strong className="mt-3 text-[9px]">เลือกภาพจากอุปกรณ์</strong><span className="mt-1 text-[7px] text-[var(--muted)]">หรือวางไฟล์ลงในพื้นที่นี้</span></label>
              )}
            </Field>
            ...
            <Field label="ใครมองเห็นเรื่องนี้ได้?" required>
              ...
              {privacy === "public" ? <div className="mt-3 flex items-start gap-2 rounded-[14px] border border-[#f0d6a9] bg-[#fff9ed] p-3 text-[8px] leading-4 text-[#8b611b]"><Info className="mt-0.5 shrink-0" size={15} /> เรื่องสาธารณะอาจปรากฏบนหน้าคอลเลกชันหลังผ่านการตรวจสอบสิทธิ์เนื้อหา คุณเปลี่ยนกลับเป็นส่วนตัวได้ทุกเมื่อ</div> : null}
            </Field>
          </div>

          <div className="mt-9 flex flex-col gap-3 border-t border-[var(--line)] pt-6 sm:flex-row sm:items-center sm:justify-between">
            {mode === "edit" ? <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full px-4 text-[9px] font-semibold text-[#a3264d] hover:bg-[#fff1f5]" onClick={() => setState("delete-confirm")} type="button"><Trash size={16} /> ลบความทรงจำ</button> : <p className="m-0 flex items-center gap-2 text-[8px] text-[var(--muted)]"><LockKey className="text-[var(--pink)]" size={14} /> บันทึกร่างอยู่บนอุปกรณ์นี้เท่านั้น</p>}
        ...
        <aside className={`${showPreview ? "block" : "hidden lg:block"}`}>
          <div className="sticky top-[102px] space-y-4">
            ...
            <div className="rounded-[24px] border border-[var(--line)] bg-white p-5"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-[14px] bg-[var(--blush)] text-[var(--pink)]"><Camera size={19} /></span><div><strong className="block text-[10px]">การ์ดที่เชื่อมอยู่</strong><span className="mt-1 block text-[7px] text-[var(--muted)]">{card.name} · {card.serial}</span></div></div><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-[var(--cream)]"><div className="h-full bg-[var(--pink)]" style={{ width: `${Math.min(100, ((card.memories + (mode === "create" ? 1 : 0)) / card.memoryGoal) * 100)}%` }} /></div><p className="mb-0 mt-2 text-[7px] text-[var(--muted)]">{mode === "create" ? `เรื่องนี้จะเป็นความทรงจำที่ ${card.memories + 1} จากเป้าหมาย ${card.memoryGoal}` : `กำลังแก้ไข 1 จาก ${card.memories} เรื่อง`}</p></div>
            <div className="rounded-[24px] bg-[var(--navy)] p-5 text-white"><Clock className="text-[#ff71a8]" size={21} /><h3 className="mb-0 mt-4 [font-family:var(--font-display)] text-[17px] tracking-[-.035em]">เรื่องเล่าไม่จำเป็นต้องสมบูรณ์</h3><p className="mb-0 mt-2 text-[8px] leading-5 text-white/50">บันทึกสิ่งที่รู้สึกตอนนี้ก่อน คุณกลับมาเติมรายละเอียด รูปภาพ หรือเปลี่ยนสิทธิ์ได้เสมอ</p></div>
          </div>
        </aside>
      </div>
...
function Field({ label, children, optional = false, required = false, supporting }: { label: string; children: React.ReactNode; optional?: boolean; required?: boolean; supporting?: string }) {
  return <div><div className="mb-2 flex items-center justify-between gap-3"><label className="text-[10px] font-bold">{label}{required ? <span className="ml-1 text-[var(--pink)]">*</span> : null}{optional ? <span className="ml-1 text-[7px] font-normal text-[var(--muted)]">ไม่บังคับ</span> : null}</label>{supporting ? <span className="text-[7px] text-[var(--muted)]">{supporting}</span> : null}</div>{children}</div>;
}
...
function PrivacyChoice({ value, active, onClick }: { value: MemoryPrivacy; active: boolean; onClick: () => void }) {
  ...
  return <button aria-pressed={active} className={`relative min-h-[120px] rounded-[18px] border p-4 text-left transition ${active ? "border-[var(--pink)] bg-[#fff4f7] shadow-[0_0_0_3px_rgba(233,30,99,.06)]" : "border-[var(--line)] bg-white hover:border-[#dfbbc8]"}`} onClick={onClick} type="button"><span className={`grid size-9 place-items-center rounded-[13px] ${active ? "bg-[var(--pink)] text-white" : "bg-[var(--cream)] text-[var(--muted)]"}`}><Icon size={17} weight={active ? "fill" : "regular"} /></span><strong className="mt-3 block text-[9px]">{privacyLabels[value].label}</strong><span className="mt-1 block text-[7px] leading-4 text-[var(--muted)]">{privacyLabels[value].description}</span>{active ? <span className="absolute right-3 top-3 grid size-5 place-items-center rounded-full bg-[var(--pink)] text-white"><Check size={11} weight="bold" /></span> : null}</button>;
}
...
function MemoryPreview({ title, body, date, location, privacy, image }: { title: string; body: string; date: string; location: string; privacy: MemoryPrivacy; image: string }) {
  ...
  return <motion.div animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[24px] border border-[var(--line)] bg-white shadow-[var(--shadow-card)]" initial={{ opacity: 0, y: 8 }}>{image ? <div className="relative aspect-[1.6]"><Image alt="ภาพตัวอย่าง" className="object-cover" fill sizes="410px" src={image} unoptimized={image.startsWith("blob:")} /></div> : <div className="grid aspect-[2.1] place-items-center bg-[var(--cream)] text-[#cbbcc2]"><ImageSquare size={32} /></div>}<div className="p-5"><div className="flex items-center justify-between gap-2"><span className="[font-family:var(--font-mono)] text-[7px] uppercase tracking-[.1em] text-[var(--pink-strong)]">{date ? new Intl.DateTimeFormat("th-TH", { dateStyle: "long" }).format(new Date(`${date}T00:00:00`)) : "ยังไม่ได้เลือกวันที่"}</span><span className="inline-flex items-center gap-1 rounded-full bg-[var(--cream)] px-2 py-1 text-[7px] text-[var(--muted)]"><Icon size={11} /> {privacyLabels[privacy].label}</span></div><h3 className="mb-0 mt-3 [font-family:var(--font-display)] text-[19px] tracking-[-.035em]">{title || "ชื่อความทรงจำ"}</h3><p className="mb-0 mt-3 line-clamp-5 text-[9px] leading-5 text-[var(--muted)]">{body || "เรื่องราวของคุณจะปรากฏตรงนี้…"}</p>{location ? <span className="mt-4 flex items-center gap-1 border-t border-[var(--line)] pt-3 text-[7px] text-[var(--muted)]"><MapPin size={11} /> {location}</span> : null}</div></motion.div>;
}
