      ...
      <OpsPanel>
        <div className="mb-5 flex flex-col gap-4 border-b border-[var(--line)] pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex gap-1 rounded-xl bg-[#f5f0f2] p-1" role="tablist" aria-label="Rewards operations">
            ...
            <button aria-selected={tab === "redemptions"} className={`min-h-10 rounded-lg px-4 text-xs font-semibold ${tab === "redemptions" ? "bg-white text-[var(--ink)] shadow-sm" : "text-[var(--muted)]"}`} onClick={() => setTab("redemptions")} role="tab" type="button">Redemptions <span className="ml-1 rounded-full bg-[var(--pink)] px-1.5 py-0.5 text-[12px] text-white">{pending}</span></button>
          </div>
            ...
            {rewards.map((reward) => {
              ...
              return (
                <article className="rounded-2xl border border-[var(--line)] p-4" key={reward.id}>
                  ...
                  <span className="mt-5 block font-[family-name:var(--font-mono)] text-[11px] tracking-[0.09em] text-[var(--pink-strong)]">{reward.type.toUpperCase()} · {reward.id}</span>
                  <h3 className="mt-1 min-h-11 font-[family-name:var(--font-display)] text-base font-bold leading-6">{reward.name}</h3>
                  <div className="mt-4 grid grid-cols-3 gap-2 border-y border-[var(--line)] py-3 text-center"><span><strong className="block text-sm">{reward.points}</strong><small className="text-[11px] text-[var(--muted)]">POINTS</small></span><span><strong className="block text-sm">{available}</strong><small className="text-[11px] text-[var(--muted)]">AVAILABLE</small></span><span><strong className="block text-sm">{reward.reserved}</strong><small className="text-[11px] text-[var(--muted)]">RESERVED</small></span></div>
