        ...
        <>
          ...
          <section className="mt-8" id="collection">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div><p className="m-0 [font-family:var(--font-mono)] text-[11px] font-bold uppercase tracking-[.14em] text-[var(--pink-strong)]">OWNED COLLECTION</p><h2 className="mb-0 mt-2 [font-family:var(--font-display)] text-[26px] font-semibold tracking-[-.045em]">การ์ดของฉัน</h2></div>
              <Link className="inline-flex min-h-10 items-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 text-[12px] font-bold transition hover:border-[var(--pink)] hover:text-[var(--pink-strong)]" href="/tap/soul_demo_7k3m9q2v"><Radio size={16} /> แตะการ์ดใบใหม่</Link>
            </div>
          ...
          <section className="mt-9 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(300px,.6fr)]">
            <div className="rounded-[28px] border border-[var(--line)] bg-white p-5 sm:p-7">
              <div className="mb-5 flex items-center justify-between gap-4">
                <div><p className="m-0 [font-family:var(--font-mono)] text-[11px] font-bold uppercase tracking-[.14em] text-[var(--pink-strong)]">RECENT STORIES</p><h2 className="mb-0 mt-2 [font-family:var(--font-display)] text-[23px] font-semibold tracking-[-.04em]">ความทรงจำล่าสุด</h2></div>
                <Link className="flex min-h-10 items-center gap-1.5 text-[12px] font-bold text-[var(--pink-strong)]" href="/member/cards/founder-088">ดู Timeline <ArrowRight size={14} /></Link>
              </div>
            ...
            <aside className="overflow-hidden rounded-[28px] bg-[var(--navy)] p-6 text-white sm:p-7">
              <div className="flex items-center justify-between"><span className="grid size-11 place-items-center rounded-2xl bg-[var(--pink)]/15 text-[#ff71a8]"><Gift size={22} weight="fill" /></span><span className="rounded-full bg-white/[0.07] px-3 py-1 [font-family:var(--font-mono)] text-[10px] uppercase tracking-[.1em] text-white/80">NEXT REWARD</span></div>
              <h2 className="mb-0 mt-10 [font-family:var(--font-display)] text-[27px] font-semibold tracking-[-.045em]">Rose Member</h2>
              <p className="mb-0 mt-2 text-[10px] leading-5 text-white/78">สะสมอีก 260 แต้ม เพื่อปลดล็อกสิทธิ์จองกิจกรรมก่อนใคร</p>
              ...
              <div className="mt-2 flex justify-between [font-family:var(--font-mono)] text-[10px] text-white/75"><span>1,240</span><span>1,500 PTS</span></div>
              ...
              <div className="relative mt-7 border-t border-white/10 pt-5"><div className="flex items-center gap-2 text-[11px] text-white/80"><ShieldCheck className="text-[#52d7a8]" size={15} /> คะแนนผูกกับบัญชี ไม่อยู่บน NFC</div></div>
            </aside>
          </section>
        </>
      )}
...
function SummaryCard({ icon: Icon, label, value, detail, tone }: { icon: typeof CardsThree; label: string; value: string; detail: string; tone: "pink" | "gold" | "cyan" }) {
  ...
  return (
    <motion.article whileHover={{ y: -3 }} className="flex min-h-[126px] items-center gap-4 rounded-[22px] border border-[var(--line)] bg-white p-4 shadow-[0_8px_30px_rgba(83,41,58,.045)] sm:p-5">
      ...
      <div className="min-w-0"><span className="block text-[11px] text-[var(--muted)]">{label}</span><strong className="mt-1 block [font-family:var(--font-display)] text-[23px] tracking-[-.04em]">{value}</strong><span className="mt-1 block truncate text-[10px] text-[#958a8f]">{detail}</span></div>
    </motion.article>
  );
}
  ...
  return (
    <motion.article animate={{ opacity: 1, y: 0 }} className="group grid overflow-hidden rounded-[28px] border border-[var(--line)] bg-white shadow-[0_14px_45px_rgba(83,41,58,.07)] sm:grid-cols-[210px_1fr]" initial={{ opacity: 0, y: 16 }} transition={{ delay: index * 0.08 }}>
      <div className={`relative min-h-[230px] overflow-hidden bg-gradient-to-br ${toneStyles[card.tone]} sm:min-h-[280px]`}>
        ...
        <span className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-[var(--navy)]/55 px-2.5 py-1 [font-family:var(--font-mono)] text-[10px] font-bold uppercase tracking-[.1em] text-white backdrop-blur-md"><CheckCircle className="text-[#5bdfaf]" size={12} weight="fill" /> Verified</span>
        <div className="absolute bottom-4 left-4 text-white"><span className="block [font-family:var(--font-mono)] text-[10px] tracking-[.12em] text-white/60">SERIAL</span><strong className="mt-1 block [font-family:var(--font-display)] text-[18px]">{card.serial}</strong></div>
      </div>
      <div className="flex min-w-0 flex-col p-5 sm:p-6">
        <span className="[font-family:var(--font-mono)] text-[10px] font-bold uppercase tracking-[.14em] text-[var(--pink-strong)]">{card.series}</span>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 border-y border-[var(--line)] py-4">
          <div><span className="block text-[10px] text-[var(--muted)]">ความทรงจำ</span><strong className="mt-1 block text-[11px]">{card.memories} เรื่อง</strong></div>
          <div><span className="block text-[10px] text-[var(--muted)]">แตะล่าสุด</span><strong className="mt-1 block text-[11px]">{card.lastTap}</strong></div>
        </div>
        <div className="mt-4"><div className="flex justify-between text-[10px] text-[var(--muted)]"><span>Memory journey</span><span>{card.memories}/{card.memoryGoal}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[var(--cream)]"><div className="h-full rounded-full bg-[var(--pink)]" style={{ width: `${progress}%` }} /></div></div>
        <div className="mt-auto flex gap-2 pt-5">
          <Link className="inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-full bg-[var(--ink)] px-4 text-[12px] font-bold text-white transition hover:bg-[var(--pink)]" href={`/member/cards/${card.id}`}>เปิดการ์ด <ArrowRight size={14} /></Link>
...
function MemoryRow({ memory }: { memory: MemberMemory }) {
  ...
  return (
    <Link className="grid min-h-[92px] grid-cols-[42px_1fr_auto] items-center gap-3 py-3 transition hover:translate-x-1" href={`/member/cards/${memory.cardId}/memories/${memory.id}/edit`}>
      ...
      <span className="min-w-0"><strong className="block truncate text-[10px]">{memory.title}</strong><span className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-[var(--muted)]"><span>{memory.displayDate}</span>{memory.location ? <span className="inline-flex items-center gap-1"><MapPin size={10} /> {memory.location}</span> : null}</span></span>
      <span className="flex items-center gap-1 rounded-full bg-[var(--cream)] px-2 py-1 text-[10px] text-[var(--muted)]"><Icon size={11} />{privacyLabels[memory.privacy].label}</span>
    </Link>
  );
}
...
function EmptyCards() {
  return (
    <div className="col-span-full flex min-h-[280px] flex-col items-center justify-center rounded-[28px] border border-dashed border-[#d9c8cf] bg-white p-8 text-center"><span className="grid size-14 place-items-center rounded-full bg-[var(--blush)] text-[var(--pink)]"><Fingerprint size={25} /></span><h3 className="mb-0 mt-5 [font-family:var(--font-display)] text-[20px]">Wallet นี้ยังไม่มีการ์ด</h3><p className="mb-0 mt-2 text-[12px] text-[var(--muted)]">แตะการ์ด SOUL หรือกรอกรหัสจากบรรจุภัณฑ์เพื่อเริ่มต้น</p><Link className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-full bg-[var(--pink)] px-5 text-[10px] font-bold text-white" href="/tap/soul_demo_7k3m9q2v"><Radio size={16} /> เริ่มแตะการ์ด</Link></div>
  );
}
