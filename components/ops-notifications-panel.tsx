      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(300px,.75fr)]">
              ...
              <article className={`rounded-2xl border p-4 transition ${item.read ? "border-[var(--line)] bg-white" : "border-[#dfb5c5] bg-[#fff8fa]"}`} key={item.id}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--line)] pt-3"><span className="text-[9px] text-[#94888e]">{item.category} · {item.createdAt}</span><span className="flex items-center gap-2"><button className="min-h-9 rounded-lg px-2 text-[9px] font-semibold text-[var(--muted)] hover:bg-white" onClick={() => void setRead(item, !item.read)} type="button">{item.read ? "Mark unread" : "Mark read"}</button><Link className="inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-[var(--navy)] px-3 text-[9px] font-semibold text-white" href={item.href}>Open <ArrowSquareOut size={13} /></Link></span></div>
                  </div>
                </div>
              </article>
            ))}
        ...
        <div className="space-y-4">
          <OpsPanel>
            ...
            ] as const).map((preference) => { const Icon = preference.icon; return <label className="flex cursor-pointer items-center gap-3 border-b border-[var(--line)] py-4 last:border-0" key={preference.key}><span className="grid size-9 place-items-center rounded-xl bg-[#f6f1f3] text-[var(--pink-strong)]"><Icon size={18} weight="duotone" /></span><span className="min-w-0 flex-1"><strong className="block text-xs">{preference.label}</strong><small className="block text-[9px] text-[var(--muted)]">{preference.detail}</small></span><input aria-label={preference.label} checked={preferences[preference.key]} className="size-5 accent-[var(--pink)]" onChange={(event) => void updatePreference(preference.key, event.target.checked)} type="checkbox" /></label>; })}
          </OpsPanel>
